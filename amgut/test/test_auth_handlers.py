...
class TestAuthBasehandler(TestHandlerBase):
    def test_set_current_user(self):
        ...
        self.mock_login(ag_data.ut_get_supplied_kit_id(
            'fbb510cb-2141-7507-e040-8a800c5d5537'))
        response = self.get('/auth/register/')
...
class TestAuthLoginHandler(TestHandlerBase):
    ...
    def test_post_correct_pass(self):
        ...
        kit_id = ag_data.ut_get_supplied_kit_id(
            'fbb510cb-2141-7507-e040-8a800c5d5537')
        self.mock_login(kit_id)
        response = self.get('/authed/portal/')
        # print("CIRR", response.headers['Set-Cookie'])
        self.assertEqual(response.code, 200)
        ...
        kit_id = ag_data.ut_get_supplied_kit_id(
            'ded5101d-c962-f6b3-e040-8a80115d6f03')
        self.mock_login(kit_id)
        response = self.get('/authed/portal/')
        self.assertEqual(response.code, 200)
    ...
    def test_post_wrong_pass(self):
        ...
        kit_id = ag_data.ut_get_supplied_kit_id(
            'fbb510cb-2141-7507-e040-8a800c5d5537')
        response = self.post('/auth/login/', {'skid': kit_id,
                                              'passwd': 'wrong'})
        self.assertIn(text_locale['handlers']['INVALID_KITID'], response.body)
        self.assertEqual(response.code, 200)
